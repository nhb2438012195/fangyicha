...
async function handleCancel() {
  ...
  try {
    ...
    const res = await orderApi.cancel(order.value.id, reason || '客户主动取消')
    order.value = res.data
...
<template>
  <div class="order-detail-page">
    <div class="page-header">
      <el-button text :icon="ArrowLeft" @click="router.push('/customer/orders')">
        返回订单列表
      </el-button>
    </div>
    ...
    <div v-if="!loading && order" class="detail-content">
      <el-card shadow="never" class="detail-card">
        ...
        <!-- 操作按钮 -->
        <div class="action-bar" v-if="order.status === '待支付'">
          <el-button type="primary" size="large" @click="handlePay">
            去支付
          </el-button>
          <el-button size="large" type="warning" @click="handleCancel">
            取消订单
          </el-button>
        </div>
      </el-card>
    ...
    <div v-if="!loading && !order" class="empty-state">
      ...
      <el-button type="primary" @click="router.push('/customer/orders')">
        返回订单列表
      </el-button>
    </div>
  </div>
